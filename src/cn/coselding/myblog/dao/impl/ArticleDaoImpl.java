...
public class ArticleDaoImpl implements ArticleDao {
    ...
    public List<Article> getPageData(String selection, Object[] params, int startindex, int pagesize) throws SQLException {
        ...
        String sql = "select artid,time,author,title,staticURL,a.cid,top,likes,looked,type,c.cname,meta from article a,category c where a.cid=c.cid order by top desc,time desc limit ?,?;";
        //params不为空，修改参数
        if(params!=null&&params.length>0){
            selection = " where "+selection;
            for(int i=0;i<params.length;i++)
                list.add(params[i]);
            sql= "select artid,time,author,title,staticURL,a.cid,top,likes,looked,type,c.cname,meta from article a,category c "+selection+" and a.cid=c.cid order by top desc,time desc limit ?,?;";
        }
